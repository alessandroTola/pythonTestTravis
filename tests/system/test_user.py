        ...
        with self.app() as client:
            with self.app_context():
                ...
                self.assertDictEqual({'message': 'User created successfully'},
                                     json.loads(response.data))  # I need to convert j response into a python dictionary
        ...
        with self.app() as client:
            with self.app_context():
                ...
                auth_response = client.post('/auth', data=json.dumps({'username': 'test', 'password': '1234'}),
                                            headers={'Content-type': 'application/json'})  # give us a token

                self.assertIn('access_token', json.loads(auth_response.data).keys())
                # keys return only the keys, in that case ['access_token']
